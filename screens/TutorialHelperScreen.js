...
class TutorialHelperScreen extends React.Component {
  pressNext() {
    if (this.props.navigation.state.params) {
      if (this.props.navigation.state.params.number === 4) {
        ...
        this.props.navigation.navigate("TutorialHelper" + this.props.navigation.state.params.number.toString(), {
  ...
  render() {
    ...
    const helpNumber =
      this.props.navigation.state.params && this.props.navigation.state.params.number ? this.props.navigation.state.params.number : 0;
    return (
      <ScrollingPageContainer style={{ backgroundColor: COLOR_TERTIARY }}>
        <ProgressStepper totalSteps={8} stepNumber={this.props.navigation.state.params.number} style={{ marginBottom: 0 }} />
        <View
          style={{
