...
import { COLOR_HIGHLIGHT, COLOR_PRIMARY, COLOR_QUATERNARY, COLOR_SECONDARY, COLOR_TERTIARY, COLOR_WHITE } from "../styles/common";
...
class AffirmationTile extends React.Component {
  ...
  render() {
    return (
      <TouchableOpacity onPress={() => this.getNewAffirmation()} style={{ flex: 1 }} activeOpacity={0.9}>
        <View style={[styles.header, { backgroundColor: this.getBackgroundColor(this.affirmationNumber) }]}>
          {!this.props.premium ? (
          ...
          <View>
            <MyText
              style={[
                ...
                !!affirmationData[this.affirmationNumber].author ? { paddingBottom: 20 } : { paddingBottom: 0 }
              ]}
            >
              {affirmationData[this.affirmationNumber].affirmation}
            </MyText>
            {!!affirmationData[this.affirmationNumber].author ? (
              <MyText style={styles.affirmationAuthor}>- {affirmationData[this.affirmationNumber].author} -</MyText>
            ) : (
              <View />
            )}
          </View>
